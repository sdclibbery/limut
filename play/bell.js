...
define(function (require) {
  ...
  return (params) => {
    ...
    let freq = scale.degreeToFreq(degree, param(params.oct, 4))/4
    let detune = param(params.detune, 0.1)

    let vca = envelope(params, 0.01)
    system.mix(effects(params, vca))

    let vco4 = fmOp(freq*21.98, 0, params)
    vco4.connect(vca)

    let vco3 = fmOp(freq*10.38, 1*detune*10, params)
    let vco3_4gain = flatEnv(700)
    vco3.connect(vco3_4gain)
    vco3_4gain.connect(vco4.detune)

    let vco2 = fmOp(freq*3.14, 3*detune*10, params)
    let vco2_4gain = simpleEnv(150, params, 3, 0.7)
    vco2.connect(vco2_4gain)
    vco2_4gain.connect(vco4.detune)

    let vco1 = fmOp(freq*19.03, 1*detune*10, params)
    let vco1_4gain = simpleEnv(10000, params, 1, 0.3)

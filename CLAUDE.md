...
- **`visual-node-internals`** — the visualsynth shader-node compiler: `px` chains, GLSL codegen and its determinism/cache-key rules, the `>>` shader seam, eval-param pass-through exemptions, texture sources, per-frame uniforms. Use when editing `draw/visualsynth/`, `draw/visualsynth.js`, or adding visual nodes.

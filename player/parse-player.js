...
define((require) => {
  ...
  assertHas({add:1,index:0}, es[0])
  assertHas({add:2,index:1}, es[1])
  assertHas({add:1,index:0}, es[2])
  assertHas({add:2,index:1}, es[3])
  ...
  assertHas({value:'b',add:1,index:2}, es[2])
  assertHas({value:'b',add:2,index:3}, es[3])
  assertHas({value:'c',add:1,index:0}, es[4]) // Index here (and below) should really be 4 (and 5), but that's a lot more complicated
  assertHas({value:'c',add:2,index:1}, es[5])

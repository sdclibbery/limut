...
define((require) => {
    ...
    if (playerId) {
      ...
      if (playerType) {
        ...
        if (patternStr.startsWith('follow')) {
          ...
          getEventsForBeat = followPlayer(patternStr.slice(6).trim(), params, playerFactory.baseParams)
        } else if (playerFactory.stopped) {
          getEventsForBeat = () => []
        } else {
          getEventsForBeat = standardPlayer(patternStr, paramsStr, playerFactory.defaultDur, player, playerFactory.baseParams)
        }
  ...
  p = parsePlayer('p test 0, x=(this.f){per:2}, f=[0:1]l4@f')
  es = p.getEventsForBeat({count:0})
  ...
  p1 = parsePlayer('p1 kal 0, foo=[0:1]l4@f')
  players.instances.p1 = p1
  p2 = parsePlayer('p2 kal 0, bar=(p1.foo){per:2}')

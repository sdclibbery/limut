...
- `section foo, length=16, next=bar, myparam=2` — define/redefine a section. `section` is a leading keyword like `set`/`preset`, matched by `sectionRegex` in `parse-line.js` before the player fallthrough. `length` defaults to 32 beats. `next` names the follow-on section. Any other params are stored on the section object and readable via expression lookup.
...
### Section blocks (section-scoped code lines)

`section drop { <body lines> }, length=16` scopes body lines to the section — they only take effect while it is active. Mechanics:

- **Accumulation** (`update-code.js` `parseCommand`): an opener line matching exactly `section name {` (nothing after the brace) collects raw lines verbatim until a line *starting* with `}` (no brace counting — lambdas in body lines are safe, but a multi-line expression whose own `}` starts a line would falsely close the block). Params after the closer can span lines. The whole block is joined with `\n` and passed to `parseLine` as one command; unterminated blocks throw.
- **Dispatch** (`parse-line.js` section branch): a line containing `\n` is the block form. Header gives the name, closer tail gives the params (always parsed, so lengths/`next` chains exist regardless of active state), body lines are grouped into commands with the same `isLineStart` continuation rule and parsed via recursive `parseLine` **only if `sections.active.name === name`** (name comparison so the built-in `default` object matches). Nested section definitions throw. Parsing a block sets `sections.hasBlocks`.
- **Auto rerun on section change**: `sections.update(beatCount)` returns true when `active` changed to a *different* object (default→default self-advance and re-forcing the current section return false). In `main.js`, when it returns true and `sections.hasBlocks` is set, `rerunForSectionChange()` (update-code.js, re-runs the remembered `latestCode` through the full `updateCode` GC cycle with `{auto:true}`) runs first and the beat's player events are played in `.then()` — so the new section's players still sound from the boundary beat. The existing GC/override machinery does the rest: players only defined in the now-inactive section are swept (destroyed), scoped overrides drop out because overrides are cleared and re-parsed each update, and `define` rebinding keeps the live section's timing across the re-parse.
- **`suppressForce`**: during auto reruns `sections.suppressForce` is set (try/finally in `updateCode`), making `forceActive`/`forceNext` no-ops — otherwise `set section.active/next=` lines in the code would refire on every section change and break self-sequencing.
- `sections.hasBlocks` is cleared in `sections.gc_reset()` and re-set during parse, so programs without blocks never trigger reruns.

### Triggering from the UI (`section/section-buttons.js`)

`set section.next=X` is a **parse-time side effect**, and `updateCode` re-parses every line from scratch on every code update (all vars/overrides are wiped and rebuilt), so such a line refires on every Ctrl-Enter — hence the row of buttons, which triggers a section exactly once per click.

- `buttonState()` is the pure core: `[{name, active, queued}]` derived from `instances`/`active`/`next`/`pendingActive` (pending counts as queued so a shift-click reads back before the next beat). `update()` diffs against what it last rendered and is called **per frame** from `main.js` `tick`, outside the `if (beat)` block; it no-ops when `#section-buttons` is absent, so module-level harness pages don't break.
- Click → `forceNext(name, true)`; click on the queued section → `sections.clearNext()`; shift-click → `forceActive(name, true)`. The second `fromUi` arg **bypasses `suppressForce`**: the auto rerun is async, so a click can land while it is set, and a click must never be swallowed. Existing DSL call sites pass no second arg and keep the suppression behaviour above.
- Handlers are on `mousedown` with `preventDefault()` so clicking never pulls focus (or the caret) out of the editor.
- The row hides itself (via the shared `.closed` class) when `default` is the only section. Note the CSS needs an explicit `p#section-buttons.closed` rule — a bare `.closed` loses to the id selector that sets `display:flex`.

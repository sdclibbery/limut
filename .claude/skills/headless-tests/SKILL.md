...
- `Assertion failed` traces only report the assert helper's line (e.g. `CONSOLE:202`), not the failing test line. To locate every failure in a module at once, temporarily make that file's local `assert` helper log its caller before tracing: `console.log('FAILAT ' + (new Error().stack.split('\n')[2]||'').trim() + ' EXP ' + x + ' ACT ' + a)`, then `grep FAILAT` the log for `file.js:line` plus expected/actual. Remove it afterwards.
- **The "Script error for:" cascade** (a module fetch refused or reset by the server) can take out whole modules, including the one you changed, so a PASSED summary may not have run your tests. `server.sh` serves with a 256 listen backlog to prevent it; a bare `python3 -m http.server` (backlog 5) is prone to it. Check your module's `"<Name> tests complete"` line is present; if not, rerun. `index.html` sets requirejs `waitSeconds: 0`, so `--virtual-time-budget` can no longer trip a load timeout.
...
- **Byte comparison is a poor test that something *is* animating.** Two grabs of a moving picture can land almost anywhere relative to each other, and a symmetric subject makes it worse: two samples of a rotating bar came back 6 differing bytes out of 1.4M and read exactly like a frozen uniform, when the rotation was working perfectly. Take **three** samples rather than two, use a subject with no rotational or mirror symmetry, and compare each render against a known-static control before concluding anything is stuck. Equivalence (differingBytes === 0) is the reliable direction; difference is not.
...
- **A clean headless run does not clear audio timing races.** Anything involving live (keyboard/midi) note timing or the audio device does not reproduce here — the null sink is far steadier than a real device. See the `verifier-audio-wiring` skill's Electron section for the numbers and the throwaway-Electron-host recipe.
- Seed code with `require('update-code').updateCode(src)` rather than `CodeMirror.setValue` when driving a *file://* page (Electron): setValue's change handler writes `localStorage['limut-code']` and clobbers the user's open set. `?nosave` skips both the load and the save handler.
- **Runs sometimes stall at startup**: `require.js` loads but `main.js` is never requested, and the log has no `INFO:CONSOLE` lines at all. Not a code problem, just rerun; a driver script should detect "no CONSOLE output after ~20s" and retry rather than wait out its full timeout.
- **Run one headless Chrome at a time.** Two launched concurrently (even with separate `--user-data-dir`) crashed at startup (`Trying to load the allocator multiple times`) and produced empty logs. Kill each by PID in the same shell command that started it: `kill %1` in a later Bash call does not reach it, and leftover instances make later runs silently log nothing.
- **Before/after comparison against HEAD without touching the working tree**: `git worktree add --detach $TMPDIR/limut-base HEAD`, serve it on 8001 from inside it with the `server.sh` command (port changed), copy the same driver into both trees, run each in turn and compare downsampled pixel dumps (log every 3rd pixel's RGB as base64 with a fixed prefix). Pin anything time-dependent to a constant (`t:5.37`, `set{z:1.3}`) or the diff is meaningless. `git worktree remove --force` and kill the 8001 server afterwards.
- **An equivalence you can read without pixels**: after one px compiles, a second spelling that generates identical source is a program cache hit and never calls `shaderSource`. A `nosrc` result for the second spelling proves the two generate the same shader.

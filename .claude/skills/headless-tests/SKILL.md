...
- **The "Script error for:" cascade** (a requirejs load timeout under `--virtual-time-budget`) can take out whole modules, including the one you changed, so a PASSED summary may not have run your tests. Check your module's `"<Name> tests complete"` line is present; if not, rerun. A loop that retries until the log has no `Script error for` and has a summary line settles it in one or two runs.

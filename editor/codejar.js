...
let CodeJar = function (editor, highlight, opt) {
  ...
  let handleSelfClosingCharacters = function(event) {
    const open = `([{'"`
    const close = `)]}'"`
  ...
  let handleTabCharacters = function(event) {
    if (event.key === "Tab") {
      preventDefault(event)
      if (event.shiftKey) {
        const before = beforeCursor()
  ...
  let handleUndoRedo = function(event) {
    if (isUndo(event)) {
      preventDefault(event)
      at--
  ...
  let isCtrl = function(event) {
    return event.metaKey || event.ctrlKey
  }

  let isUndo = function(event) {
    return isCtrl(event) && !event.shiftKey && event.code === "KeyZ"
  }

  let isRedo = function(event) {
    return isCtrl(event) && event.shiftKey && event.code === "KeyZ"
  }
